...
public class RagService {
    ...
    public void queryStream(String question, List<Map<String, String>> history, StreamCallback callback) {
        try {
            ...
            List<VectorSearchService.SearchResult> searchCandidates =
                vectorSearchService.searchSimilarDocuments(question, candidateTopK);
            retrievalEvaluationService.evaluateAndLog(question, "rag_candidate", candidateTopK, searchCandidates);

            // 2. 重排与过滤，降低低相关上下文造成的幻觉
            List<VectorSearchService.SearchResult> searchResults =
                ragRerankService.rerankAndFilter(question, searchCandidates, topK);
            retrievalEvaluationService.evaluateAndLog(question, "rag_final", topK, searchResults);
